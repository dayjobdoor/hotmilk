...
| `defaults.persona`                | Seeds `.pi/gentle-ai/persona.json` when missing. Supported modes: `gentleman`, `neutral`, `gyal`, and `raiden`; hotmilk default is `neutral`. `gyal` and `raiden` replace gentle-pi's persona section while preserving its harness rules. |
...
`/mode` starts with **Defaults** (`persona`), then bundled-extension sections from `BUNDLED_EXTENSION_GROUP_ORDER` in `src/config/bundled-extensions.ts`:

| Group                     | Extensions (toggle ids)                                                                                                    |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **Defaults**              | `persona`: `gentleman`, `neutral`, `gyal`, `raiden`                                                                        |
| **Harness**               | `skill-registry`, `sdd-init`, `gentle-ai`                                                                                  |
| **Agent tools**           | `ask-user`, `todo`, `graphify`, `shazam`, `prompt-template-model`, `subagents`, `herdr-squad`, `lens`, `fff`, `web-access` |
| **Context & performance** | `context-mode`, `simplify`, `rtk-optimizer`, `observational-memory`, `engram`                                              |
| **Integrations**          | `goal`, `docparser`, `obsidian`, `btw`, `intercom`, `mcp-adapter`                                                          |
