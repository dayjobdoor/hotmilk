...
3. Implementation steps with concrete `verify:` checks per [docs/testing.md](../../../docs/testing.md) Verification.

## Execute (Phase 4 chat path)

Work plan steps in order. Each step ends with its `verify:` check.
...
2. Run verify from [docs/testing.md](../../../docs/testing.md) (e.g. `bun run test`) — record pass/fail.
3. Say: "gentle-ai unavailable — inline execute; SDD and delegation skipped."
```

**Verify:** use [docs/testing.md](../../../docs/testing.md) Verification (`bun run test`, `bun run lint`, `bun run check`). Record pass/fail per step.
